...
export default function TaskSelectScreen() {
    ...
    const initializeTask = useMutation(api.tasks.initializeTask);

    const handleSelect = async (taskId: string) => {
        const taskDetails = taskData.tasks.find(t => t.id.toString() === taskId);

        if (taskDetails && matchId) {
            // Convert options to TaskItems
            const initialItems = taskDetails.options?.map((text: any) => ({
                id: Math.random().toString(36).substr(2, 9),
                text: typeof text === 'string' ? text : (text.label || JSON.stringify(text)),
                timestamp: Date.now()
            })) || [];
            ...
            await initializeTask({
                ...
                taskId,
                type: taskDetails.type || 'shared_list',
        ...
        router.replace({
            pathname: '/task/[id]',
            params: { id: matchId, taskId, count: '1', level: '1' }
        });
    };
