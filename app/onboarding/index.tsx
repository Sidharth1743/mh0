...
export default function WelcomeScreen() {
    ...
    const { animatedStyle: foxAnimatedStyle, react, message: foxMessage, scale: foxScale } = useBuddyAnimations();
    const foxUnfurl = useSharedValue(0);
    const getOrCreateUser = useMutation(api.users.getOrCreateUser);

    const onFoxPress = () => {
        react('happy', "You've just unlocked your first connection spark! ✨");
    ...
    const handleGetStarted = async () => {
        try {
            ...
            const userId = await getOrCreateUser({ deviceId, nickname: 'User' });
            console.log('User created/fetched:', userId);

            // Save Convex userId for later use (like linking to Supabase)
            if (userId) {
                await AsyncStorage.setItem('convex_user_id', userId);
            }

            console.log('Navigating to value-prop...');
